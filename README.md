...
### プライム上場銘柄に AI 推論を一括投入(seed/本番初期データ生成)

本番 D1 で `company_ai_brief` 系がほぼ空のときに、プライム上場銘柄に対して 8 AI タスク (`ai-summary` / `ai-logo-color` / `ai-stock-trend` / `ai-valuation` / `ai-catalysts` / `ai-positioning` / `ai-forecast` / `ai-market-brief`) を一括実行し、結果を `scripts/seed/snapshots/` に書き出すコマンドです。

```bash
# 段階検証 (memory: ai-task-codes-pitfall — 必ず単体 dry-run から)
pnpm seed:fill-ai --codes 7203 --tasks ai-summary --skip-snapshot --yes   # ① 疎通
pnpm seed:fill-ai --codes 7203 --skip-snapshot --yes                       # ② 全 8 タスク
pnpm seed:fill-ai --limit 10 --skip-snapshot --yes                         # ③ レート確認
pnpm seed:fill-ai --limit 10 --yes                                         # ④ snapshot まで
pnpm seed:fill-ai                                                           # ⑤ 全件 (ETA 確認後)
```
...
- `--tasks ai-summary,…` 一部タスクのみ (default: 全 8)
- `--skip-snapshot` snapshot 書き出しを省略
- `--yes` / `-y` コスト確認プロンプトを skip

実行前に「推定 Claude CLI 呼び出し回数 / 所要時間」を表示し、`--yes` 無ければ y/N 待ち。1 件失敗で次へ継続し、失敗銘柄は `local/tmp/seed-fill-ai-failed.json` に記録。全タスクで succeeded=0 のときのみ snapshot 書き出しを skip(空 snapshot 焼き付け事故防止。memory: yahoo-rate-limit-pitfall)。

本番反映は別ターン手動:

```bash
git diff scripts/seed/snapshots/                           # 差分レビュー
pnpm db:restore                                            # 確認後、ローカル D1 を復元
pnpm tsx scripts/dump-tables-to-remote.ts \
  company_ai_brief stock_snapshot predictions market_brief story_decks story_slides
```
