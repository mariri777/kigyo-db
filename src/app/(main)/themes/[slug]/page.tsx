  ...
  return (
    <article className="max-w-6xl mx-auto px-6 py-8">
      ...
      <header className="border-b border-border pb-10 mb-12">
        ...
        <p className="text-muted-foreground leading-relaxed max-w-3xl text-base mb-8">{theme.lede}</p>
        ...
        <div className="grid sm:grid-cols-2 md:grid-cols-4 gap-px bg-border border border-border rounded-md overflow-hidden">
          {theme.macroContext.map((m) => (
            <div key={m.label} className="bg-background p-4">
              <div className="text-[10px] text-foreground/60 tracking-wider mb-1">{m.label}</div>
            ...
            >
              <div className="flex items-baseline justify-between mb-2 gap-2">
                <div className="flex items-baseline gap-2">
                  <span className="text-[10px] text-foreground/60 font-mono tabular">#{idx + 1}</span>
                  <span className="text-[10px] text-foreground/60 font-mono tabular">{p.stock.code}</span>
                  <h3 className="text-lg font-bold tracking-tight group-hover:underline">
                    {p.stock.name}
                  </h3>
                </div>
                <span className="text-[10px] text-foreground/60 shrink-0">{p.stock.sectorTSE}</span>
              </div>
              <div className="text-[11px] text-muted-foreground tabular mb-3">
      ...
      <section className="mb-16 max-w-3xl">
        <div className="mb-8">
          ...
          <p className="text-sm text-muted-foreground">マクロ・構造的な背景と投資論点</p>
        </div>
        ...
        <div className="space-y-8">
          {theme.sections.map((sec) => (
            <div key={sec.heading}>
              ...
              <p className="text-[14px] leading-relaxed text-muted-foreground">{sec.body}</p>
            </div>
          ))}
        </div>
      </section>
        ...
        <div className="border border-border rounded-md overflow-hidden">
          <table className="w-full text-sm">
            <thead className="bg-surface border-b border-border">
              <tr>
                <th className="text-left px-4 py-3 text-[10px] text-foreground/60 font-bold tracking-wider w-12">
                  #
                </th>
                <th className="text-left px-4 py-3 text-[10px] text-foreground/60 font-bold tracking-wider">
                  銘柄
                </th>
                <th className="text-right px-4 py-3 text-[10px] text-foreground/60 font-bold tracking-wider">
                  {theme.rankLabel}
                </th>
                <th className="text-right px-4 py-3 text-[10px] text-foreground/60 font-bold tracking-wider hidden sm:table-cell">
                  PER
                </th>
                <th className="text-right px-4 py-3 text-[10px] text-foreground/60 font-bold tracking-wider hidden md:table-cell">
                  配当
                </th>
                <th className="text-right px-4 py-3 text-[10px] text-foreground/60 font-bold tracking-wider hidden md:table-cell">
                  時価総額
            ...
            <tbody>
              {ranked.map((s, idx) => {
                ...
                return (
                  ...
                  >
                    <td className="px-4 py-3 text-[10px] text-foreground/60 font-mono tabular">
                      #{idx + 1}
                    </td>
                    <td className="px-4 py-3">
                      <Link href={`/stocks/${s.code}`} className="hover:underline">
                        <span className="font-mono tabular text-[10px] text-foreground/60 mr-2">
                          {s.code}
                        </span>
                    ...
                    <td className="px-4 py-3 text-right tabular text-muted-foreground hidden sm:table-cell">
                      {formatPerOpt(s.per)}
                    </td>
      ...
      <section className="mb-16 max-w-3xl">
        ...
        <ul className="space-y-2">
          {theme.risks.map((r) => (
            <li key={r} className="text-[13px] leading-relaxed text-muted-foreground flex gap-2">
      ...
      <section className="mb-16 grid md:grid-cols-2 gap-8">
        {relatedIndustries.length > 0 && (
          <div>
            <h3 className="text-sm font-bold tracking-wider text-muted-foreground mb-3 uppercase">
              関連業界
            </h3>
            <div className="space-y-2">
              {relatedIndustries.map((ind) => (
                ...
                >
                  ...
                  <span className="text-[11px] text-muted-foreground ml-2">→ 業界マップ</span>
                </Link>
              ))}
            </div>
          </div>
            ...
            <div className="space-y-2">
              {relatedPosts.map((p) => (
                <Link
                  ...
                  className="block bg-surface border border-border rounded-md p-3 hover:border-border-strong transition"
                >
                  <span className="text-[10px] text-foreground/60 tracking-wider mr-2 uppercase">
                    {p.category}
                  </span>
      ...
      {/* ===== Disclaimer ===== */}
      <section className="border-t border-border pt-6 text-[11px] text-foreground/60 leading-relaxed">
        <p>
