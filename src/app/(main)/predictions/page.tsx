          ...
          <section key={b} className="mb-12" id={`bucket-${b}`}>
            <div className="flex items-end justify-between mb-4 pb-2 border-b border-border">
              <div>
                ...
                <p className="text-[12px] text-muted-foreground mt-1">{meta.sub}</p>
              </div>
              <span className="text-[11px] text-foreground/60">{items.length} 件</span>
            </div>
...
function Stat({ label, value, sub }: { label: string; value: string; sub?: string }) {
  return (
    <div className="bg-surface border border-border rounded-md px-3 py-2.5">
      ...
      {sub && <div className="text-[10px] text-foreground/60 mt-1.5">{sub}</div>}
    </div>
  );
}
