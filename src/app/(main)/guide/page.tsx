        ...
        <section>
          ...
          <div className="bg-surface border border-border rounded-md p-4 space-y-3 text-sm">
            <div>
              ...
              <span className="text-muted-foreground ml-2">金融庁の電子開示システム。有価証券報告書・四半期報告書・大量保有報告書を取得</span>
            </div>
            <div>
              <strong>TDnet</strong>
              <span className="text-muted-foreground ml-2">東京証券取引所の適時開示情報伝達システム。決算短信・適時開示資料を取得</span>
            </div>
            <div>
              <strong>各社 IR ページ</strong>
              <span className="text-muted-foreground ml-2">決算説明会資料・統合報告書・中期経営計画を確認</span>
            </div>
          </div>
        </section>
        ...
        <section className="border-t border-border pt-10">
          ...
          <div className="grid sm:grid-cols-2 gap-3">
            ...
            >
              ...
              <div className="text-[12px] text-muted-foreground mt-1">ADC・Hi-NA・パテントクリフ・Rule of 40 など</div>
            </Link>
            <Link
              href="/stocks/8035"
              className="block bg-surface border border-border rounded-md p-4 hover:border-border-strong transition group"
            >
              <div className="text-[10px] text-foreground/60 tracking-widest mb-1">具体的な銘柄ページの例</div>
              <div className="font-bold group-hover:underline">東京エレクトロン（8035）→</div>
              <div className="text-[12px] text-muted-foreground mt-1">半導体製造装置の世界トップ企業</div>
            </Link>
            <Link
              href="/industries/semiconductor"
              className="block bg-surface border border-border rounded-md p-4 hover:border-border-strong transition group"
            >
              <div className="text-[10px] text-foreground/60 tracking-widest mb-1">業界マップの例</div>
              <div className="font-bold group-hover:underline">半導体業界マップ →</div>
              <div className="text-[12px] text-muted-foreground mt-1">バリューチェーン・競争構造・KPI を一望</div>
            </Link>
            <Link
              href="/screens"
              className="block bg-surface border border-border rounded-md p-4 hover:border-border-strong transition group"
            >
              <div className="text-[10px] text-foreground/60 tracking-widest mb-1">切り口で探す</div>
              <div className="font-bold group-hover:underline">スクリーン一覧 →</div>
              <div className="text-[12px] text-muted-foreground mt-1">割安・高配当・拡大期など 7 つの切り口</div>
            </Link>
          </div>
        </section>
      </div>
      ...
      <ol className="space-y-2">
        {steps.map((step, i) => (
          <li key={i} className="grid grid-cols-[140px_1fr] gap-3 items-baseline">
            ...
            <span className="text-[12px] text-muted-foreground leading-relaxed">{step.descNode ?? step.desc}</span>
          </li>
        ))}
      </ol>
    </div>
