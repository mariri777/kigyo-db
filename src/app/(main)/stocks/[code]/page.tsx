    ...
    <article className="max-w-6xl mx-auto px-6 py-8">
      ...
      <header className="border-b border-border pb-6 mb-8">
        ...
        <details className="mb-3 group">
          ...
          <div className="text-[10px] text-muted-foreground leading-relaxed mt-2 pl-3 border-l border-border max-w-3xl space-y-1">
            <div>
        ...
        <details className="mt-2 group">
          ...
          <div className="text-[10px] text-muted-foreground leading-relaxed mt-2 pl-3 border-l border-border max-w-3xl grid sm:grid-cols-2 gap-x-4 gap-y-1">
      ...
      <section className="border-t border-border pt-6 mt-12">
        <details className="text-xs text-foreground/60 leading-relaxed">
          ...
          <div className="mt-3 space-y-2 pl-2 border-l-2 border-border">
            <p>
              <strong className="text-muted-foreground">定量データ</strong>：EDINET XBRL から決定的に取得。日付・出典をすべて記録。AI は数値を生成しません。
            </p>
            <p>
              <strong className="text-muted-foreground">事業タグ</strong>：有報の「事業の内容」「セグメント情報」と決算説明会資料・統合報告書を LLM で構造化。タグマスター語彙は半導体クラスタで先行設計。
            </p>
            <p>
              <strong className="text-muted-foreground">類似度スコア</strong>：A 軸はタグ jaccard 加重、C 軸はフェーズ連続スコアの cos 類似、D 軸はファクター感応度ベクトルの距離。全て決定的計算。
            </p>
            <p>
              <strong className="text-muted-foreground">論点抽出</strong>：三段階プロンプト（差分抽出 → 影響要素フィルタ → 非自明性フィルタ）で汎用論点と既知論点を排除。引用検証パイプラインで根拠未確認の出力は非表示。
            </p>
          </div>
        </details>
      </section>
