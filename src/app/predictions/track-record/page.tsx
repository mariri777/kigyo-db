  ...
  return (
    <article className="max-w-5xl mx-auto px-6 py-12">
      ...
      <section className="mb-14">
        ...
        <p className="text-sm text-muted mb-3 leading-relaxed max-w-2xl">
        ...
        <p className="text-[11px] text-dim mb-6 leading-relaxed max-w-2xl">
      ...
      <section className="mt-16 pt-8 border-t border-border max-w-2xl">
        ...
        <div className="space-y-3 text-[13px] text-muted leading-relaxed">
          ...
          <p>
            ...
            <Link href="/legal/disclaimer" className="text-foreground underline decoration-dotted underline-offset-2 hover:text-muted mx-1">
              免責事項
            </Link>
            ・
            <Link href="/legal/editorial-policy" className="text-foreground underline decoration-dotted underline-offset-2 hover:text-muted">
              編集方針
            </Link>
            をご覧ください。
          </p>
...
function BigStat({
  ...
  return (
    <div className={`bg-surface border rounded-md px-4 py-3 ${highlight ? "border-foreground" : "border-border"}`}>
      <div className="text-[10px] text-dim font-bold tracking-[0.15em] uppercase mb-1">{label}</div>
...
function BarTable({ rows, emptyHint }: { rows: TrackRecordRow[]; emptyHint: string }) {
  return (
    <div className="bg-surface border border-border rounded-md overflow-hidden">
      <div className="grid grid-cols-[1fr_auto_auto_120px] gap-4 px-4 py-2 text-[10px] text-dim tracking-wider border-b border-border bg-surface-elev">
        <div>区分</div>
      ...
      {rows.map((row) => (
        ...
        >
          ...
          <div className="text-right tabular font-mono w-16 text-muted">
          ...
          <div className="text-right">
            {row.total > 0 ? (
              ...
              <span className="text-dim text-[11px]">{emptyHint.length > 30 ? "サンプル不足" : "—"}</span>
            )}
          </div>
        </div>
      ))}
