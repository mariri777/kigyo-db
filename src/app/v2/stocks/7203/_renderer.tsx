  ...
  return (
    <nav className="text-xs text-neutral-500 flex items-center gap-1.5 flex-wrap">
      <Link href="/v2" className="hover:text-neutral-900 inline-flex items-center gap-1">
        <ChevronLeft className="w-3.5 h-3.5" />v2 ホーム
      </Link>
      <span>/</span>
      <Link href={`/v2#industries`} className="hover:text-neutral-900">{data.industryName}</Link>
      <span>/</span>
    ...
    <section>
      ...
      <div className="mt-5 grid grid-cols-2 sm:grid-cols-3 lg:grid-cols-5 gap-3">
        {peers.map((p) => {
          ...
          return (
            <Link key={p.code} href={`/v2/stocks/${p.code}`} className="group bg-white rounded-xl shadow-sm hover:shadow-md hover:-translate-y-0.5 transition-all p-4 flex flex-col gap-1.5">
