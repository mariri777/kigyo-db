  ...
  return (
    <article className="max-w-3xl mx-auto px-6 py-12">
      <header className="pb-8 border-b border-border mb-10">
        <p className="text-muted text-xs font-bold tracking-[0.2em] uppercase mb-3">{eyebrow}</p>
        <h1 className="text-4xl sm:text-5xl font-bold leading-tight tracking-tighter mb-5">
          {title}
        </h1>
        {intro && <p className="text-muted leading-relaxed text-base max-w-2xl">{intro}</p>}
        <div className="mt-6 text-[11px] text-dim">最終更新：{effectiveDate}</div>
      </header>
      ...
      <footer className="mt-16 pt-8 border-t border-border flex flex-wrap gap-4 text-sm">
        ...
        <Link href="/" className="text-muted hover:text-foreground transition ml-auto">
          トップへ →
        </Link>
      </footer>
    </article>
