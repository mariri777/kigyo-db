        ...
        {FACTOR_ANOMALIES.map((a) => (
          ...
          >
            <div className="flex items-baseline justify-between mb-3">
              <div>
                <div className="text-[10px] text-foreground/60 tabular">{a.code}</div>
                <div className="text-base font-bold leading-tight group-hover:underline">{a.name}</div>
              </div>
              <span className="shrink-0 text-[10px] text-muted-foreground border border-border bg-surface-elev rounded px-1.5 py-0.5">
                {a.factor}
              </span>
            </div>
            <div className="space-y-2 mb-3">
              <div className="text-[11px]">
                <div className="text-foreground/60 mb-0.5">教科書：</div>
                <div className="text-muted-foreground line-through decoration-dim/40">{a.expected}</div>
              </div>
              <div className="text-[11px]">
                <div className="text-foreground/60 mb-0.5">実際：</div>
                <div className="text-foreground font-medium">{a.actual}</div>
              </div>
            </div>
            <p className="text-[11px] text-muted-foreground leading-relaxed border-t border-border pt-2">{a.insight}</p>
          </Link>
        ))}
      </div>
    </section>
