...
//
// 使い方：
//   node scripts/update-prices.mjs template
//     → data/prices.csv を現在の株価で生成（これを編集する）
//
//   node scripts/update-prices.mjs apply --date 2026-06-12
...
function loadStockBlocks() {
  const src = readFileSync(DATA_TS, "utf8");
  // code: "XXXX" の位置で data.ts を銘柄ブロックに分割する
...
function replaceOnce(segment, regex, replacement, label, code) {
  const matches = segment.match(new RegExp(regex.source, regex.flags + "g"));
  if (!matches || matches.length !== 1) {
    throw new Error(`${code}: ${label} の置換位置が一意に特定できません（${matches?.length ?? 0} 件）`);
  }
...
function cmdApply(args) {
  ...
  if (suspicious.length && !force) {
    ...
    for (const s of suspicious) {
      console.error(`   ${s.code} ${s.name}: ${s.oldPrice} → ${s.price} (${s.pct.toFixed(1)}%)`);
    }
  ...
  const sorted = [...changes].sort((a, b) => byCode.get(b.code).start - byCode.get(a.code).start);
  for (const c of sorted) {
    ...
    seg = replaceOnce(seg, /marketCapOku: -?[\d.]+,/, `marketCapOku: ${Math.round(mcap * r)},`, "marketCapOku", c.code);
    seg = replaceOnce(seg, /\bper: -?[\d.]+,/, `per: ${(per * r).toFixed(1)},`, "per", c.code);
    seg = replaceOnce(seg, /\bpbr: -?[\d.]+,/, `pbr: ${(pbr * r).toFixed(2)},`, "pbr", c.code);
    seg = replaceOnce(seg, /dividendYield: -?[\d.]+,/, `dividendYield: ${(yld / r).toFixed(2)},`, "dividendYield", c.code);
  ...
  console.log(`✅ ${changes.length} 銘柄を ${date} 付で反映（価格変更 ${updated.length}・変わらず ${flat}）`);
  for (const c of updated) {
    const arrow = c.pct >= 0 ? "↑" : "↓";
    console.log(`   ${c.code} ${c.name}: ${c.oldPrice.toLocaleString()} → ${c.price.toLocaleString()} 円 (${arrow}${Math.abs(c.pct).toFixed(2)}%)`);
  }
  if (missing.length) {
...
if (cmd === "template") cmdTemplate();
  ...
  console.log("使い方: node scripts/update-prices.mjs template | apply --date YYYY-MM-DD [--force]");
  process.exit(1);
}
