...
```bash
# code が D1 にあるか
pnpm exec wrangler d1 execute cho-kigyo-db-database --local --json --command "SELECT s.code, c.id AS company_id, c.name FROM stocks s JOIN companies c ON c.id = s.company_id WHERE s.code = '<CODE>'"
```
...
| `ceoName` | 「代表者」最新の社長名 |
| `website` | 公式 URL |
| `employeesConsolidated` | 「従業員数」連結数値(整数) |
...
pnpm exec wrangler d1 execute cho-kigyo-db-database --local --command "UPDATE companies SET name_en = '...', founded = '...', listed = '...', headquarters = '...', ceo_name = '...', website = '...', employees_consolidated = ..., logo_color = '...', description = '...', one_liner = '...', updated_at = '$NOW' WHERE id = <COMPANY_ID>"
```

**文字列内のシングルクオートは '' でエスケープ**(SQLite 標準)。長文は `--file` 経由で SQL ファイルから投入してもよい。
...
```bash
# 既存を削除
pnpm exec wrangler d1 execute cho-kigyo-db-database --local --command "DELETE FROM financials_annual WHERE company_id = <COMPANY_ID>"

# 10 期 INSERT
pnpm exec wrangler d1 execute cho-kigyo-db-database --local --file /tmp/stock-fill-<CODE>-financials.sql
```
...
- `target_consensus` / `target_high` / `target_low` / `analyst_buy/hold/sell`(アナリスト目標、Wiki に無ければ Yahoo Finance 検索結果 from WebFetch で補完)
...
...
SQL
pnpm exec wrangler d1 execute cho-kigyo-db-database --local --file /tmp/stock-fill-<CODE>-story.sql
```
...
最後に以下を実行して、各テーブルに行が入ったことを確認:

```bash
pnpm exec wrangler d1 execute cho-kigyo-db-database --local --json --command "
...
最後にユーザーに「`http://localhost:3000/v2/stocks/<CODE>` を開いて確認してください」と伝える。
